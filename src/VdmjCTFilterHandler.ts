...
export class VdmjCTFilterHandler implements CTFilterHandler {
    ...
    private showFilterOptions(): void {
        ...
        this._filterKeyTypes.forEach((v, k) =>
            showOptions.push(
                v +
                    ": " +
                    (k == "trace reduction type" ? this._traceReductionTypes.get(this._filters.get(k).toString()) : this._filters.get(k))
            )
        );
        ...
        vscode.window.showQuickPick(showOptions).then((res) => {
            if (res == undefined || res == "OK") {
                ...
                this._inSetup = false;
                return;
            } else if (res == "Reset") this.resetFilters();
            else {
                let filterKey = this._filterKeyTypesReverse.get(res.substring(0, res.indexOf(":")));
                if (filterKey == "trace reduction type") this.showReduction();
                else if (filterKey == "trace filtering seed") this.showSeed();
                else if (filterKey == "subset limitation") this.showLimit();
            }
        });
    }
    ...
    private showSeed() {
        let inputOptions: vscode.InputBoxOptions = {
            ...
            validateInput: (input) => {
                ...
                if (Number.isNaN(num)) return "Invalid input: Not a number";

                if (!Number.isInteger(num)) return "Invalid input: Not an integer";

                if (num > 0) return undefined;
                else return "Invalid input: Not a positive integer";
            },
        };
    ...
    private showLimit() {
        let inputOptions: vscode.InputBoxOptions = {
            ...
            validateInput: (input) => {
                ...
                if (1 <= num && num <= 100) return undefined;
                else return "Invalid input: Not between 1-100";
            },
        };
        vscode.window.showInputBox(inputOptions).then((res) => {
            if (res == undefined) return;
