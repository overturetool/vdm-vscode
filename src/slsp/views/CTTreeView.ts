...
export class CTTreeView {
    ...
    constructor(private _ctHandler: CTHandler, private _context: ExtensionContext, private _canFilter: boolean = false) {
    ...
    public getNumberOftests(traceName: string): number {
        return [].concat(...this._combinatorialTests.map((symbol) => symbol.traces)).find((trace) => trace.name == traceName).testCases
            .length;
    }
    ...
    public saveCTs() {
        this._combinatorialTests.forEach((ct) => {
            // Create full path
            let savePath = util.joinUriPath(
                this._ctHandler.currentClient.dataStoragePath,
    ...
    private async loadCTs(): Promise<completeCT[]> {
        return new Promise(async (resolve, reject) => {
            ...
            fs.access(savePath, fs.constants.F_OK | fs.constants.R_OK, (accessErr) => {
                ...
                if (accessErr.code === "ENOENT") return resolve(completeCTs);
    ...
    private testExecutionFinished() {
        if (!this._executingTests) return;
        ...
        if (
            !this._isExecutingTestGroup &&
            !this._executeCanceled &&
            this._currentlyExecutingTrace.testCases.length > this._numberOfUpdatedTests
        )
            this._currentlyExecutingTrace.testCases.splice(
                this._numberOfUpdatedTests,
                this._currentlyExecutingTrace.testCases.length - this._numberOfUpdatedTests
            );
        ...
        if (this._currentlyExecutingTrace != undefined) {
            ...
            else this._currentlyExecutingTrace.verdict = null;
        }
    ...
    public async addNewTestResults(traceName: string, testCases: CTTestCase[]) {
        if (this._currentlyExecutingTrace.name != traceName) return;
        ...
        for (let i = 0; i < testCases.length; i++) {
            ...
            else this._currentlyExecutingTrace.testCases.push(newTestCase);
        }
        // Handle if user has executed all test groups manually.
        if (
            this._isExecutingTestGroup &&
            testCases[testCases.length - 1].id ==
                this._currentlyExecutingTrace.testCases[this._currentlyExecutingTrace.testCases.length - 1].id
        ) {
    ...
    private async ctRebuildOutline(useCurrentClient: boolean = false) {
        if (this._isRebuildingTraceOutline) return;
        ...
        if (!useCurrentClient || this._ctHandler.currentClient == undefined) didSelect = await this._ctHandler.showAvailableSpecsForCT();
    ...
    private matchLocalSymbolsToServerSymbols(serverSymbols: CTSymbol[], localSymbols: completeCT[]): completeCT[] {
        return serverSymbols.map((serverSymbol) => {
            ...
            if (!localSymbol)
                return {
                    symbolName: serverSymbol.name,
                    traces: serverSymbol.traces.map((trace) => {
    ...
    private async ctGenerate(traceViewElement: TestViewElement) {
        if (traceViewElement.type != TreeItemType.Trace) return;
    ...
    private async generate(traceViewElement: TestViewElement) {
        ...
        let traceWithTestResults: traceWithTestResults = []
            .concat(...this._combinatorialTests.map((symbol) => symbol.traces))
            .find((trace) => trace.name == traceViewElement.label);
        ...
        try {
            ...
            if (traceWithTestResults.testCases.length != numberOfTests) {
                ...
                for (let i = 1; i <= numberOfTests; i++) traceWithTestResults.testCases.push({ id: i, verdict: null, sequence: [] });
            }
    ...
    private async ctGoToTrace(traceViewElement: TestViewElement) {
        ...
        let traceLocation: Location = []
            .concat(...this._combinatorialTests.map((symbol) => symbol.traces))
            .find((trace) => trace.name == traceViewElement.label).location;
        if (!traceLocation) return;
    ...
    private onDidExpandElement(viewElement: TestViewElement) {
        ...
        if (viewElement.type == TreeItemType.TestGroup) this._testProvider.rebuildViewFromElement(viewElement);
    }
    ...
    private onDidChangeSelection(viewElement: TestViewElement) {
        if (viewElement.type == TreeItemType.Test)
            ...
            this._resultProvider.setTestSequenceResults(
                []
                    .concat(...this._combinatorialTests.map((symbol) => symbol.traces))
                    .find((trace) => trace.name == viewElement.getParent().getParent().label)
                    .testCases.find((testResult) => testResult.id + "" == viewElement.label).sequence
            );
    }
    ...
    private updateUI() {
        this._testProvider.rebuildViewFromElement(
            []
                .concat(...this._testProvider.getRoots().map((symbolViewElement) => symbolViewElement.getChildren()))
                .find((traceViewElement) => traceViewElement.label == this._currentlyExecutingTrace.name)
        );
    }
                ...
                return new Promise<void>(async (resolve, reject) => {
                    try {
                        ...
                        if (viewElement.type == TreeItemType.Trace) {
                            ...
                            this._currentlyExecutingTrace = []
                                .concat(...this._combinatorialTests.map((symbol) => symbol.traces))
                                .find((trace) => trace.name == viewElement.label);
                            ...
                            this._currentlyExecutingTrace = []
                                .concat(...this._combinatorialTests.map((symbol) => symbol.traces))
                                .find((trace) => trace.name == viewElement.getParent().label);
