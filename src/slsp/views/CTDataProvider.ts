...
export class CTTreeDataProvider implements TreeDataProvider<TestViewElement> {
    ...
    constructor(private _ctView: CTTreeView, private _context: ExtensionContext) {
    ...
    getChildren(element?: TestViewElement): Thenable<TestViewElement[]> {
        ...
        if (element.type == TreeItemType.CTSymbol) {
            ...
            element.setChildren(
                ctTraces.map((trace) => {
                    let traceViewElement = new TestViewElement(
                        trace.name,
                        TreeItemType.Trace,
                        TreeItemCollapsibleState.Collapsed,
                        "",
                        element,
                        this.verdictToIconPath(trace.verdict),
                        trace.verdict
                    );
        ...
        if (element.type == TreeItemType.Trace) {
            ...
            for (let i = 0; i < groups; i++) {
                let testRange: NumberRange = {
                    start: 1 + i * this.groupSize,
                    end: this.groupSize >= numberOfTests ? numberOfTests + this.groupSize * i : this.groupSize * (i + 1),
                };
                ...
                for (let k = 0; k < results.length; k++) {
                    if (!toShow && this._verdictKindToShow.includes(results[k].verdict))
                        // if we find, in a group, a verdict that corresponds to a _verdictKindToShow(verdict that we want to show) then we can have to show this group (if the group contains at least 1 desired verdict then the group is selected)
                        toShow = true;
        ...
        if (element.type == TreeItemType.TestGroup) {
            ...
            for (let i = 0; i < testsResults.length; i++) {
                if (!this._filter || this._verdictKindToShow.includes(testsResults[i].verdict))
                    // filter tests and show only the ones whick correspond to the right verdict
                    testsViewElements.push(
                        new TestViewElement(
                            testsResults[i].id + "",
                            TreeItemType.Test,
...
export class TestViewElement extends TreeItem {
    ...
    constructor(
        ...
        if (description === "") super.description = false;
        else super.description = description;
