...
var vdmDialect = 'vdmpp'
var serverModule = 'lsp.LSPServerSocket'

export function activate(context: ExtensionContext) {
	let options = { cwd: workspace.rootPath};
	let logPath = context.logPath;
	//let clientLogFile = path.resolve(logPath, '..', 'vdmpp_lang_client.log');
	let clientLogFile = path.resolve(context.extensionPath, 'vdmpp_lang_client.log');
	//let serverLogFile = path.resolve(logPath, '..', 'vdmpp_lang_server.log');
	let serverLogFile = path.resolve(context.extensionPath, 'vdmpp_lang_server.log');
	...
	let ClientOptions: LanguageClientOptions = {
		...
		documentSelector: [{ language: 'VDM_PP'}],
		synchronize: {
			// Setup filesystem watcher for changes in vdmpp files
...
class VdmConfigurationProvider implements vscode.DebugConfigurationProvider {
	...
	resolveDebugConfiguration(folder: WorkspaceFolder | undefined, config: DebugConfiguration, token?: CancellationToken): ProviderResult<DebugConfiguration> {
		...
		if (!config.type && !config.request && !config.name) {
			...
			if (editor && editor.document.languageId === 'VDM_PP') {
				config.type = 'vdm';
				config.name = 'Launch';
