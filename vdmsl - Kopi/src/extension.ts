...
export function activate(context: ExtensionContext) {
	...
	//let clientLogFile = path.resolve(logPath, '..', 'vdmsl_lang_client.log');
	let clientLogFile = path.resolve(context.extensionPath, 'vdmsl_lang_client.log');
	//let serverLogFile = path.resolve(logPath, '..', 'vdmsl_lang_server.log');
	let serverLogFile = path.resolve(context.extensionPath, 'vdmsl_lang_server.log');
	...
	let ClientOptions: LanguageClientOptions = {
		...
		documentSelector: [{ language: 'VDM_SL'}],
		synchronize: {
			// Setup filesystem watcher for changes in vdmsl files
...
class VdmConfigurationProvider implements vscode.DebugConfigurationProvider {
	...
	resolveDebugConfiguration(folder: WorkspaceFolder | undefined, config: DebugConfiguration, token?: CancellationToken): ProviderResult<DebugConfiguration> {
		...
		if (!config.type && !config.request && !config.name) {
			...
			if (editor && editor.document.languageId === 'VDM_SL') {
				config.type = 'vdm';
				config.name = 'Launch';
